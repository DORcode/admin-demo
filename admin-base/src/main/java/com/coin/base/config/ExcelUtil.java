...
public class ExcelUtil {
     ...
     * @throws
     * @MethodName export
     * @Description 导出上报半周报明细
     * @author kh
     * @date 2020/7/8 14:07
     */
    public static boolean export(HttpServletResponse response, List<Map> data) {
        Workbook wb = new XSSFWorkbook();
...
//        }
        ...
        try (OutputStream os = response.getOutputStream()) {
            for (int i = 0; i < data.size(); i++) {
                ...
                titleRow.setHeightInPoints((short) 18);
                Cell titleCell = titleRow.createCell(colNum, CellType.STRING);
...
//                }
            ...
            Cell leftTitleCell = sheet.getRow(0).createCell(0, CellType.STRING);
            leftTitleCell.setCellStyle(headStyle);
            leftTitleCell.setCellValue("工作类别");

            String orgName = "";
            if (CollectionUtils.isNotEmpty(data)) {
                Set<String> orgNames = new HashSet<>();
                for (Map m : data) {
                    String temp = (String) m.get("orgName");
                    orgNames.add(temp);
                }
                orgName = (String) data.get(0).get("orgName");
                if (orgNames.size() > 1) {
                    orgName = "空间公司";
                }
            }
            ...
            if (StringUtils.isEmpty(orgName)) {
                ...
                wbName = String.format("%s周报", orgName);
            }
